...
def main(
    ...
    for i in glob.glob(os.path.join(data_dir, 'train_images', '*')):
        ...
        train_generator[-1].append(
            np.moveaxis(gdal_array.LoadFile(i), 0, -1).astype('int16')
        )
...
def train(
    ...
    val_image = np.moveaxis(
        gdal_array.LoadFile(
            os.path.join(
                data_dir, 'train_images/s01_20190423_0_0_1024_1024.tif'
            )
        ),
        0,
        -1,
    ).astype('int16')
    val_mask = gdal_array.LoadFile(
        os.path.join(data_dir, 'train_masks/s01_20190423_0_0_1024_1024.tif')
    ).astype('int8')
    ...
    if pred_all.shape[1] == 1:
        ...
        detection_decoded = np.argmax(
            pred_all.reshape(dim, dim, pred_all.shape[1]), axis=-1
        ).reshape(dim, dim)
...
def visualize_detections(
    ...
    for i in range(0, 1):  # np.shape(detections)[0]):
        ...
        if detections[i].shape[1] == 1:
            ...
            print(
                categorical_dice(
                    onehot_encode(ground_truths[i], id2code),
                    onehot_encode(detection_decoded, id2code),
                )
            )
            print(
                dice_coef(
                    ground_truths[i].reshape(1, dim, dim),
                    detection_decoded.reshape(1, dim, dim),
                )
            )
        else:
            detection_decoded = np.argmax(
                detections[i].reshape(dim, dim, detections[i].shape[1]), axis=-1
            ).reshape(dim, dim, 1)
            print(
                categorical_dice(
                    onehot_encode(ground_truths[i], id2code),
        ...
        plt.savefig(
            os.path.join(out_dir, geoinfos[i][0][:-4]), bbox_inches='tight'
        )
        plt.close()
